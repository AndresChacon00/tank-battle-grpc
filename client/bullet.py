...
class Bullet(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        colliding_blocks = pygame.sprite.spritecollide(self, self.collision_group, False)
        solid_collision = next((block for block in colliding_blocks if getattr(block, "solid", False)), None)

        if solid_collision:
            if self.bounces > 0:
                # Rebote: invertir la dirección según el eje de colisión
                if abs(self.rect.centerx - solid_collision.rect.centerx) > abs(self.rect.centery - solid_collision.rect.centery):
